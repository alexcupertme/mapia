...
type PrimitiveOf<T extends PrimitiveTag> =
...
export type DeepCastTypes<T, From, To, Preserve extends object = BuiltinAtomic> =
  [T] extends [From] ? To :
  T extends readonly (infer U)[] ? DeepCastTypes<U, From, To>[] :
  T extends Preserve ? T :
  T extends object
  ? ([From] extends [undefined]
    ? {
      [K in keyof T]-?: DeepCastTypes<
        undefined extends T[K]
        ? Exclude<T[K], undefined> | To
        : T[K],
        From,
        To
      >
...
export function deepCastTypes<T, F extends PrimitiveTag, G extends PrimitiveTag>(
  value: T,
  from: F,
  to: G
): DeepCastTypes<T, PrimitiveOf<F>, PrimitiveOf<G>>;
  ...
  const convert = (v: any): any => {
    if (typeof to === "string") {
      switch (to) {
        ...
        case "symbol": return typeof v === "symbol" ? v : Symbol(String(v));
        case "undefined": return undefined;
        case "null": return null;
      }
    }
    return new (to as Ctor)(v);
  };
