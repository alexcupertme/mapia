type ReplaceKeysWithParsed<T, Suffix extends string, From, To> =
...
export type ReplaceKeysStringOrNumber<T, Suffix extends string, To> =
...
export function parseStringOrNumberFieldsEndsWith<T, Suffix extends string, To>(
  ...
  if (!isObjectOrArray(value)) return value as ReplaceKeysStringOrNumber<T, Suffix, To>;
  if (!hasAnyKeyEndingWith(value, endsWith)) return value as ReplaceKeysStringOrNumber<T, Suffix, To>;
  ...
  const walk = (v: unknown): unknown => {
    ...
    if (v && typeof v === "object") {
      ...
      for (const [k, cur] of Object.entries(v as Record<string, unknown>)) {
        if (k.endsWith(endsWith) && (typeof cur === "string" || typeof cur === "number")) {
          out[k] = new Ctor(cur);
        } else if (k.endsWith(endsWith) && (cur === null || cur === undefined)) {
          out[k] = cur;
        } else {
          out[k] = walk(cur);
        }
